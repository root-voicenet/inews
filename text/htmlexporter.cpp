...
//     kDebug() << "html" << html;
    ...
    if ( charFormat.hasProperty( NvTextFormat::HasCodeStyle ) &&
         charFormat.boolProperty( NvTextFormat::HasCodeStyle ) ) {
        tags << code;
...
//    html += QLatin1String("\"");
        ...
        if ( format.hasProperty( NvTextFormat::AnchorTitle ) ) {
            const QString title = format.stringProperty( NvTextFormat::AnchorTitle );
            if ( !title.isEmpty() ) {
        ...
        if ( format.hasProperty( NvTextFormat::AnchorTarget ) ) {
            const QString target = format.stringProperty( NvTextFormat::AnchorTarget );
            if ( !target.isEmpty() ) {
...
//     kDebug() << txt ;
    if ( txt.count() == 1 && txt.at( 0 ) == QChar::ObjectReplacementCharacter ) {
        if ( format.isImageFormat() ) {
            ...
            if ( imgFmt.hasProperty( NvTextFormat::ImageTitle ) ) {
                const QString title = imgFmt.stringProperty( NvTextFormat::ImageTitle );
                if ( !title.isEmpty() ) {
                    emitAttribute( "title", imgFmt.stringProperty( NvTextFormat::ImageTitle ) );
                }
            }

            if ( imgFmt.hasProperty( NvTextFormat::ImageAlternateText ) ) {
                const QString alternate = imgFmt.stringProperty( NvTextFormat::ImageAlternateText );
                if ( !alternate.isEmpty() ) {
                    emitAttribute( "alt", imgFmt.stringProperty( NvTextFormat::ImageAlternateText ) );
                }
            }
...
void HtmlExporter::emitBlock( const QTextBlock &block )
{
    ...
    if ( blockFormat.hasProperty( NvTextFormat::IsBlockQuote ) &&
         blockFormat.boolProperty( NvTextFormat::IsBlockQuote ) ) {
        isBlockQuote = true;
    }
...
//     const QTextBlockFormat blockFormat = block.blockFormat();
    if ( blockFormat.hasProperty( QTextFormat::BlockTrailingHorizontalRulerWidth ) ) { 
        if ( ( blockFormat.hasProperty( NvTextFormat::IsHtmlTagSign ) ) &&
            ( blockFormat.boolProperty( NvTextFormat::IsHtmlTagSign ) ) ) {
...
//         html += QLatin1Char( '>' );
        ...
        if (!list) {
            ...
            if ( ( blockFormat.hasProperty( NvTextFormat::HtmlHeading ) ) && (
                blockFormat.intProperty( NvTextFormat::HtmlHeading ) ) ) {
                const int index = blockFormat.intProperty( NvTextFormat::HtmlHeading );
...
//             html += QLatin1String( "</li>\n" );
//         } else {
//             if ( blockFormat::boolProperty( NvTextFormat::IsHtmlHeading ) ) {
