...
NvTextHtmlImporter::NvTextHtmlImporter( QTextDocument *_doc, const QString &_html, const QTextDocument *resourceProvider )
...
void NvTextHtmlImporter::import()
{
//     kDebug();
    ...
    for ( int i = 0; i < count(); ++i ) {
        const NvTextHtmlParserNode *node = &at( i );
        /*
        ...
        */
        if ( i > 0 && ( node->parent != i - 1 ) ) {
            ...
            if ( blockTagClosed && !hasBlock && !node->isBlock && !node->text.isEmpty() && node->displayMode != NvTextHtmlElement::DisplayNone ) {
        ...
        if ( node->displayMode == NvTextHtmlElement::DisplayNone ) {
            ...
            if ( node->background.style() != Qt::NoBrush ) {
                ...
                const_cast<NvTextHtmlParserNode *>( node )->background = QBrush();
            }
            ...
            if ( node->id == Html_ul && !node->hasOwnListStyle && node->parent ) {
                const NvTextHtmlParserNode *n = &at( node->parent );
                while ( n ) {
                    if ( n->id == Html_ul ) {
/// me       style = nextListStyle(node->listStyle);
            ...
            /// my code
            if ( nodeFmt.hasProperty( NvTextFormat::AnchorTitle ) )
                fmt.setProperty( NvTextFormat::AnchorTitle, nodeFmt.property( NvTextFormat::AnchorTitle ) );
            if ( nodeFmt.hasProperty( NvTextFormat::AnchorTarget ) )
                fmt.setProperty( NvTextFormat::AnchorTarget, nodeFmt.property( NvTextFormat::AnchorTarget ) );

            if ( !node->imageTitle.isEmpty() )
                fmt.setProperty( NvTextFormat::ImageTitle, QVariant( node->imageTitle ) );
            if ( !node->imageAlternateText.isEmpty() )
                fmt.setProperty( NvTextFormat::ImageAlternateText,
            ...
            QTextBlockFormat blockFormat;
            blockFormat.setProperty( NvTextFormat::IsHtmlTagSign, true );
        ...
        if ( node->isBlock ) {
            ...
            if ( node->isListItem ) {
                if ( node->parent && at( node->parent ).isListStart ) {
                    ...
                    const NvTextHtmlParserNode *list = &at( listId );
...
bool NvTextHtmlImporter::closeTag( int i )
{
//     kDebug();
    const NvTextHtmlParserNode *closedNode = &at( i - 1 );
...
NvTextHtmlImporter::Table NvTextHtmlImporter::scanTable( int tableNodeIdx )
{
    Table table;
    table.columns = 0;
    ...
    foreach( int row, rowNodes ) {
        ...
        foreach( int cell, at( row ).children ) {
            ...
            if ( at( cell ).isTableCell ) {
                const NvTextHtmlParserNode &c = at( cell );
    ...
    const NvTextHtmlParserNode &node = at( tableNodeIdx );
    if ( node.isTextFrame ) {
        ...
        fmt.setMargin( node.uncollapsedMargin( NvTextHtmlParser::MarginTop ) );
    } else {
...
void NvTextHtmlImporter::appendBlock( const QTextBlockFormat &format, QTextCharFormat charFmt )
{
//     kDebug();
    if ( setNamedAnchorInNextOutput ) {
